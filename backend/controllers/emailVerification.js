...
emailVerificationRouter.post("/send", async (request, response, next) => {
...
// Verify code
emailVerificationRouter.post("/verify", async (request, response, next) => {
  try {
    ...
    if (!verificationDocument) {
      return response.status(401).json({ error: "no verification code found" });
    }

    if (verificationDocument.expiresAt < Date.now()) {
      return response.status(401).json({ error: "verification code expired" });
    }
